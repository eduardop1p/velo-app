...
export default async function fetchGetFullCryptos() {
  const resCryptos = await fetch(
    `${process.env.CRYPTO_API_URL}&fsyms=BTC,DOGE,XLM,XRP,LTC,ETH,ADA,SOL,MATIC,OP,DOT,AVAX,ALGO,USDC,USDT,LINK,SAND,MANA,CRV,LDO,AAVE,UNI,MKR,SNX,COMP,QNT,ARB,ATOM,APE`,
    {
      method: 'GET',
      next: {
        revalidate: 60,
